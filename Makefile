...
.PHONY: all test clean deploy upgrade fund help install snapshot format anvil scopefile
...
ifeq ($(findstring --network sepolia,$(ARGS)),--network sepolia)
	NETWORK_ARGS := --rpc-url $(SEPOLIA_RPC_URL) --account $(SEPOLIA_DEPLOYER_NAME) --broadcast --verify --etherscan-api-key $(ETHERSCAN_API_KEY) -vvvvv
endif

deploy:
	@forge script DeployBox $(NETWORK_ARGS)
